...
const
    Signal   = require('adverb-signals'),  // event bus
    YAML     = require('yamljs'),          // output parser for OS X
    chokidar = require('chokidar'),        // file system watcher
    assert   = require('chai').assert,
    dangit   = require('dangit'),          // helper utilities
    space    = dangit.space,
    quote    = dangit.quote,
    event    = require('./event'),
    isOn     = require('./is-on'),
...
const
    cliArg = {
        ...
        win32 : {  // NOTE: No Windows support yet. Pull requests welcome!
            get     : '',
            set     : '',
            enable  : '',
            disable : space(
...
function handleError(api) {
    ...
    return function onError(err) {
        ...
        const
            ...
            reason = code ? `Exit code ${code}.` : 'No output to parse.';
...
const osProxy = {
    ...
    clear,     // delete proxy config from storage
